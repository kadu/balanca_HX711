...
input, select{background:#2a2a2a;color:white;border:1px solid #444;padding:10px;width:100%;border-radius:5px;margin-top:5px;box-sizing:border-box}
...
<div id='mqttModal' class='modal'><div class='modal-content'><h2>Configuração MQTT</h2>
...
function openMqttModal(){fetch('/getMqttConfig').then(r=>r.json()).then(d=>{document.getElementById('mq_srv').value=d.srv;document.getElementById('mq_usr').value=d.usr;document.getElementById('mq_pass').value=d.pas;document.getElementById('mq_top').value=d.top;document.getElementById('mq_en').checked=d.en=='1';document.getElementById('mqttModal').style.display='flex';})}
...
d.innerHTML=`<strong>${type=='POUR'?'DESPEJAR':'AGUARDAR'}</strong>`+
`<div class='step-controls'><button onclick='moveStep(this,"UP")'>▲</button><button onclick='moveStep(this,"DOWN")'>▼</button></div>`+
...
fetch(`/saveMqtt?srv=${s}&usr=${u}&pas=${p}&top=${t}&en=${e}`).then(()=>{alert('Salvo!');closeModal('mqttModal');})};
...
public:
    ...
    void begin() {
        ...
        server.on("/getRecipes", [this](){
            server.send(200, "application/json", recipeManager.getRecipesJson());
        });
        ...
        server.on("/deleteRecipe", [this](){
            recipeManager.deleteRecipe(server.arg("name"));
            server.send(200, "text/plain", "OK");
        });
        ...
        server.on("/setRecipe", [this](){ 
            ...
            if (!found) {
                Recipe livre; livre.name = name;
                monitor.setRecipe(livre);
            }
            server.send(200, "text/plain", "OK"); 
        });
